  ...
  const formik = useFormik({
    ...
    validate: (values) => {
      ...
      requiredFields.forEach((field) => {
        ...
        const value = keys.reduce((acc, key) => acc?.[key], values);
        if (!value) {
          if (!errors[keys[0]]) errors[keys[0]] = {};
          errors[keys[0]][keys[1]] = `Field ${keys[1]} wajib diisi`;
        }
      });
      ...
      // Validasi khusus untuk noBPJS_KIS
      if (values.noBPJS_KIS && !/^\d+$/.test(values.noBPJS_KIS)) {
        errors.noBPJS_KIS = 'Field noBPJS_KIS harus berupa angka';
      }
  ...
  return (
    <div>
      ...
      <Modal
        ...
        title={`Detail Pasien ${dataDetailPatient?.namaLengkap}`}
